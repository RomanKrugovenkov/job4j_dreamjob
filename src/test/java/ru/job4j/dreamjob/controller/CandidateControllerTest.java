...
class CandidateControllerTest {
    ...
    @Test
    public void whenRequestCandidateCreationPageThenGetPageWithCandidates() {
    ...
    @Test
    public void whenRequestOneCandidatePageThenGetPageWithOneCandidate() {
        var candidate1 = new Candidate(1, "name1", "desc1", now(), 1, 2);
    ...
    @Test
    public void whenRequest1CandidatePageThenGetErrorPageWithMessage() {
        var expectedException = new RuntimeException("Резюме не найдено");
        when(candidateService.findById(anyInt())).thenThrow(expectedException);
        var model = new ConcurrentModel();

        assertThatThrownBy(() -> candidateController.getById(model, 1))
                .isInstanceOf(Exception.class)
                .hasMessageContaining("Резюме не найдено");
    }

    @Test
    public void whenPostUpdateCandidateThenGetPageWithVacancies() throws IOException {
        var candidate1 = new Candidate(1, "name1", "desc1", now(), 1, 2);
        var candidateUpdate = new Candidate(2, "name2", "desc2", now(), 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var candidateArgumentCaptor = ArgumentCaptor.forClass(Candidate.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
        when(candidateService.findById(anyInt())).thenReturn(Optional.of(candidate1));
        ...
        var model = new ConcurrentModel();
        var view = candidateController.update(candidateUpdate, testFile, model);
        var actualcandidate = candidateArgumentCaptor.getValue();
        var actualFileDto = fileDtoArgumentCaptor.getValue();
    ...
    @Test
    public void whenRequestDeleteCandidateThenGetPageWithVacancies() throws IOException {
        var candidate1 = new Candidate(1, "name1", "desc1", now(), 1, 2);
        when(candidateService.findById(anyInt())).thenReturn(Optional.of(candidate1));
