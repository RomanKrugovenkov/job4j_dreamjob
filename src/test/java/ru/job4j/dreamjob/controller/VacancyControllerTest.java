...
public class VacancyControllerTest {
    ...
    @Test
    public void whenRequestVacancyCreationPageThenGetPageWithVacancies() {
    ...
    public void whenRequest1VacancyPageThenGetErrorPageWithMessage() {
        ...
        when(vacancyService.findById(anyInt())).thenThrow(expectedException);
        var model = new ConcurrentModel();

        assertThatThrownBy(() -> vacancyController.getById(model, 1))
    ...
    public void whenPostUpdateVacancyThenGetPageWithVacancies() throws IOException {
        var vacancy1 = new Vacancy(1, "test1", "desc1", now(), true, 1, 1);
        var vacancyUpdate = new Vacancy(1, "test2", "desc2", now(), true, 2, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
        when(vacancyService.findById(anyInt())).thenReturn(Optional.of(vacancy1));
        ...
        var model = new ConcurrentModel();
        var view = vacancyController.update(vacancyUpdate, testFile, model);
        var actualVacancy = vacancyArgumentCaptor.getValue();
        var actualFileDto = fileDtoArgumentCaptor.getValue();
    ...
    @Test
    public void whenRequestDeleteVacancyThenGetPageWithVacancies() throws IOException {
        var vacancy1 = new Vacancy(1, "test1", "desc1", now(), true, 1, 1);
        when(vacancyService.findById(anyInt())).thenReturn(Optional.of(vacancy1));
